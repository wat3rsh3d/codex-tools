---
name: drawio-skill
description: Use when the user explicitly requests a draw.io file, an editable architecture/UML/ER/network diagram, draw.io layout repair, or PNG/SVG/PDF export from draw.io. Do not use for ordinary explanations, generic planning, or a small diagram that Mermaid can express clearly.
license: MIT
...
## Diagram Composition Standard

Treat the diagram as an instrument for understanding, not a decorated inventory of boxes. A good diagram should make the reader's first action obvious, reveal structure before detail, and show why the relationships matter.

### Planner And Validator Contract

This skill is both the planner and the validator. Do not let XML generation happen before composition has been designed and checked.

Before writing any `.drawio` XML, create a brief internal layout plan with:

- Reader question and audience.
- Chosen diagram type and dominant layout grammar.
- Included nodes, omitted/summarized nodes, and why.
- Node classes and visual encodings for each class.
- Relation types and visual encodings for each relation type.
- Groups, lanes, containers, boundaries, and reading order.
- Expected grid: rows/columns, major whitespace corridors, and edge-routing strategy.
- Label budget: max lines per node, long-label breaks, connector-label rules, and shape sizing assumptions.
- Split decision: one diagram, multiple pages, or summary/detail pair.

Validate that plan before generation:

- If there is no clear entry point, exit/outcome, or dominant reading path, fix the plan before generating.
- If the diagram has too many concepts for one view, split or summarize before generating.
- If multiple meanings share one visual encoding, assign distinct encodings before generating.
- If one meaning has inconsistent shapes/icons/colors, normalize it before generating.
- If edge crossings are likely from the chosen grouping, revise the grouping before using waypoints.
- If labels cannot fit inside reasonable shapes, shorten labels or move detail to notes before generating.
- If the diagram would be a node cloud, use Graphviz/autolayout or redesign as lanes/layers before generating.

When validating an existing diagram, start with this same contract: inspect the current arrangement, identify the plan it seems to imply, list where the rendered diagram violates the plan, then repair the plan before editing XML.

Before placing shapes, define:

- **Reader question:** what should someone understand or decide after 10 seconds?
- **Audience and fidelity:** executive map, implementation diagram, recovery/debugging trace, data model, sequence, or handoff artifact.
- **Primary reading path:** left-to-right for pipelines, top-to-bottom for lifecycles/hierarchies, swimlanes for handoffs, concentric/radial only for true center-out concept maps.
- **Node classes:** actors, systems, services, data stores, policy gates, external dependencies, artifacts, states, risks, and outputs.
- **Relation types:** call, data flow, ownership, dependency, state transition, approval, fallback, containment, or evidence/provenance.
- **Emphasis:** the 1-3 elements that need visual weight; everything else supports them.
- **Split boundary:** if the diagram needs more than ~15 meaningful nodes, more than 3 semantic layers, or more than 2 relation types, group aggressively or split into focused pages/subdiagrams.

### Layout Grammar

Choose one dominant grammar and make every placement serve it:

| Grammar | Use for | Arrangement rule |
|---|---|---|
| Pipeline | sequential processing, ingestion, release, CI/CD | Inputs left, transforms middle, outputs right; failures/fallbacks below the main path |
| Layered architecture | app/service/data/control boundaries | User surfaces top/left, orchestration/control center, domain services, persistence/infrastructure bottom/right |
| Swimlane handoff | teams, roles, adapters, services taking turns | One lane per owner/boundary; arrows cross lanes only when responsibility changes |
| Lifecycle/state | status, governance, incident flow | Stable states in order; decisions as diamonds between states; terminal states visually distinct |
| Hub/spoke | one true orchestrator, registry, broker, or event bus | Hub central; spokes grouped by role; avoid hub layouts for ordinary many-to-many graphs |
| Matrix/quadrant | comparison, capability map, risk/priority | Axes must be meaningful; items align to explicit categories, not decoration |
| Dense dependency graph | imports, call graphs, many peer dependencies | Use Graphviz autolayout and then curate labels/groups; do not hand-place node clouds |

### Composition Rules

- Establish a clear entry point and exit/outcome. The reader should not have to hunt for where the diagram starts.
- Group by semantics first, then by geometry. Containers and swimlanes should represent real boundaries, owners, lifecycle phases, trust zones, or data domains.
- Keep like things visually consistent: the same shape/color/icon means the same kind of thing everywhere; different meanings need different encodings.
- Use color to encode purpose, state, owner, risk, or layer. Avoid generic AI palettes, decorative gradients, and arbitrary multi-color boxes.
- Make labels economical: nouns for nodes, verbs or protocols for edges, short state names for statuses. Move explanation to notes outside the diagram instead of stuffing paragraphs into shapes.
- Treat text containment as non-negotiable. No word or label may overlap another label, cross a shape boundary, touch the border, hide under an edge, or spill outside its component in the exported image.
- Preserve whitespace as routing space. Dense diagrams should be organized, not merely compressed.
- Prefer orthogonal edges, lane/corridor routing, and explicit entry/exit points. Edges must not cut through unrelated nodes or labels.
- Avoid crossing lines by changing grouping first, position second, waypoints third. If crossings remain, make the crossing intentional and visually legible.
- Put cross-cutting concerns such as policy, audit, security, observability, or governance in a band or lane rather than scattering repeated boxes.
- Use icons only when they add fast recognition. Do not reuse one icon for different meanings or use different icons for the same meaning.

### Text Fit Rules

Do not rely on draw.io's editor view as proof that labels fit. Verify the exported PNG/SVG, because wrapping and font metrics can differ.

- Default every text-bearing shape to `whiteSpace=wrap;html=1;` and add padding with `spacing=8;` or side-specific spacing when labels approach borders.
- Size shapes from the label, not the other way around. Increase width/height before reducing font size.
- Break long labels with `&#xa;` at semantic boundaries. Do not allow one long word or token to force spillover.
- Keep labels to 1-3 short lines inside normal nodes. If a concept needs a sentence, use a note/callout outside the main flow.
- Keep connector labels short and placed in open corridors. If an edge label overlaps an edge, node, or other label, move the route or shorten the label.
- Never place text directly on top of icons unless the icon shape is designed for a label. Put the label below or beside the icon with enough fixed space.
- On self-check, zoom mentally to the final reading size. If text only fits when zoomed in inside draw.io, the diagram is not ready.

### Arrangement Bans

Do not ship diagrams with:

- Randomly even-spaced boxes with no reading path.
- A cloud of nodes connected by diagonal spaghetti.
- Multiple relation types that look identical.
- Containers that are only decorative backgrounds.
- Repeated labels like "service", "process", "system", or "component" when precise names exist.
- Giant whitespace caused by arbitrary coordinates.
- Edges that pass through nodes, labels, or arrowheads.
- Labels that overlap, clip, spill outside shapes, or become unreadable at the exported size.
- Color choices that look like a default AI dashboard instead of an intentional legend.
- A diagram that is complete but less understandable than a bullet list.

### Composition QA

Before showing the user, answer these checks from the rendered image, not only the XML:

- Can a first-time reader identify what to look at first?
- Is the direction of flow or relationship unambiguous?
- Are boundaries, owners, states, trust zones, or layers visually obvious where relevant?
- Are similar elements encoded consistently across shape, color, icon, line, and label style?
- Are edge crossings minimized and routed through whitespace corridors?
- Are labels short enough to scan without opening draw.io, and fully contained inside their owning shapes?
- Would the diagram become clearer if split into lanes, groups, pages, or a summary/detail pair?
- Does every color/icon/container communicate meaning rather than decoration?
...
**Windows / Codex Desktop note:** Native Windows installs often expose the CLI as
`draw.io` / `draw.io.exe`, not `drawio`. Check both PATH and known install paths:
`Get-Command draw.io`, `Get-Command draw.io.exe`,
`Test-Path "$env:LOCALAPPDATA\Programs\draw.io\draw.io.exe"`, then
`Test-Path "C:\Program Files\draw.io\draw.io.exe"`. If `where.exe` or a
non-escalated PATH read disagrees with those checks, trust the direct path check
or the registry-backed user PATH from `[Environment]::GetEnvironmentVariable('Path','User')`.
For directory creation in Windows PowerShell, use
`New-Item -ItemType Directory -Force -Path <dir>`; `New-Item -LiteralPath` is
not consistently available on older Windows PowerShell builds.

For optional Graphviz auto-layout on Windows, verify `dot` with
`& "C:\Program Files\Graphviz\bin\dot.exe" -V` when `dot -V` is not on PATH.
If installed through winget as `Graphviz.Graphviz`, add
`C:\Program Files\Graphviz\bin` to the user PATH for future Codex sessions.
...
5. **Validate rendered draft** — use the agent's built-in vision capability to read the exported PNG, catch obvious issues, and run Composition QA before showing user (requires a vision-enabled model such as Claude Sonnet/Opus). Check whether the rendered diagram fulfills the pre-generation plan: arrangement quality, reading path, semantic grouping, edge routing, color/icon consistency, and label economy, not only rendering errors. If validation fails, revise the plan or XML and re-export before showing the user. If reading the PNG returns a 400 / "Could not process image" error, you almost certainly exported with `-e` by mistake — re-export without `-e` and retry once. If it still fails, skip image-based self-check and continue to step 6 with the unresolved validation risk stated.
...
| Text spillover | Words extend outside the shape, touch borders, overlap another label, or sit under an edge/icon | Increase shape size, add `spacing=8;`, insert `&#xa;` line breaks, shorten the label, or move detail to a note |
